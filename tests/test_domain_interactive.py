...
def test_every_default_profile_is_offered_and_model_validated(source):
    """Recommendations remain offered and executable, without preset bans."""
    ...
    assert offering, f"no runner route declares {profile} for {source}"
    switches = single_domain_runtime_switches(profile)
    if source == "hrrr":
        assert route_physics_problems(switches) == []
        # Surface/PBL/radiation/cumulus choices use common model validation.
        # The retired route allowlists must not be recreated by this test.
        for candidate in payload["templates"]:
            try:
                custom = single_domain_runtime_switches(candidate)
            except ValueError:
                continue
            if custom["mp_physics"] != 28:
                assert route_physics_problems(custom) == []
    assert switches["ra_lw_physics"] != 0 and switches["ra_sw_physics"] != 0
