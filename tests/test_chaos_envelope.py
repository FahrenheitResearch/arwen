...
def self_contained_campaign_authority(tmp_path_factory):
    """Keep real config validation; these identity tests never decode weather.

    Preserve the campaign's exact geometry/physics and provide owned file
    declarations for the production loader's existence checks. The ensemble
    fields below are separately generated by n5s_delegation_fixture.
    """
    from gpuwm.branch import emit_experiment_toml

    root = tmp_path_factory.mktemp("envelope-authority")
    raw = tomllib.loads(CONFIG.read_text(encoding="utf-8"))
    case = raw["case_data"]
    def declared(name):
        path = root / name
        path.write_text("Identity-only input declaration; no meteorology is decoded.\n", encoding="utf-8")
        return path.as_posix()
    case["forcing"] = [declared("forcing.grb")]
    case["vtable"] = declared("Vtable")
    case["wps_namelist"] = declared("namelist.wps")
    case["source_orography"] = {key: declared(f"{key}.nc") for key in case["source_orography"]}
    geog = root / "geog"
    geog.mkdir()
    case["geog_root"] = geog.as_posix()
    config = root / "campaign.toml"
    config.write_text(emit_experiment_toml(raw), encoding="utf-8")
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(sys.modules[__name__], "CONFIG", config)
        yield


def test_identity_loader_still_refuses_an_absent_declared_input(tmp_path):
    from gpuwm.branch import emit_experiment_toml
    raw = tomllib.loads(CONFIG.read_text(encoding="utf-8"))
    raw["case_data"]["forcing"] = [(tmp_path / "absent.grb").as_posix()]
    config = tmp_path / "missing-input.toml"
    config.write_text(emit_experiment_toml(raw), encoding="utf-8")
    with pytest.raises(ValueError, match="forcing file .* does not exist"):
        chaos_envelope.config_identity(config)
...
def test_the_registration_hash_covers_start_time_and_the_evaluator():
    """audit ver-02-03: the siblings of ``parameters`` are registered too.

    ``start_time`` selects WHICH FRAMES are scored -- ``score_pair`` hands
    it to ``discover_frames``, which keys every frame by
    ``int((valid - start).total_seconds())``.  Editing it back one hour
    re-points every registered lead at an earlier history frame, before
    convective initiation, where the runs still agree.  While the digest
    covered ``parameters`` alone that edit revalidated cleanly and the
    receipt went on quoting the pre-registered sha, so a reader binding
    the receipt to the committed registration could not see that a
    different set of frames had been scored.  ``evaluator_commit`` -- the
    "who measured this" the receipt reproduces -- was editable the same
    way, and ``schema`` is what ``validate_registration`` matches first.
    """
    registration = _registration()
    for field, value in (
            ("start_time", "1974-04-03T12:00:00"),
            ("evaluator_commit", "b" * 40),
            ("schema", "gpuwm.chaos-envelope-registration/v0"),
    ):
        tampered = json.loads(json.dumps(registration))
        tampered[field] = value
        with pytest.raises(ValueError, match="hash does not match|schema"):
            chaos_envelope.validate_registration(tampered)

    # The digest cannot cover itself, and an untouched registration still
    # validates -- so the gate above is a refusal, not a blanket one.
    assert chaos_envelope.validate_registration(registration) is not None
    reordered = json.loads(json.dumps(registration))
    reordered = {key: reordered[key] for key in reversed(list(reordered))}
    assert (chaos_envelope.validate_registration(reordered)[
