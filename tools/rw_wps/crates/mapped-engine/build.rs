...
//!
//! This crate is gpuwm's, not the donor's: `crates/rw-wps` and
//! `vendor/` are the verbatim snapshot VENDOR.md describes, and
//! `crates/mapped-engine` is the engine written here.  So it is stamped
//! the way every other gpuwm-authored bridge is, and the vendored
//! neighbour is not stamped at all.
//!
//! Resolution order:
...
/// The crate lives at `crates/<name>` inside the engine workspace; the
...
fn head_of_clean_checkout() -> Option<String> {
    ...
    // do not enter the build of tracked sources and are ignored.  The
