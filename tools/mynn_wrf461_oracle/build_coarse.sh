#!/usr/bin/env bash
# Build and run the DX > 5 km MYNN surface-layer oracle.
#
# Same contract as build.sh: the WRF checkout must be the pinned v4.6.1 commit
# and phys/module_sf_mynn.F must hash to the pinned bytes, so the CSV this
# writes is produced by the UNMODIFIED module.  It builds only the coarse
# harness and never touches surface-layer.csv, surface-layer-wide.csv or
# surface-layer-wrapper.csv.
set -euo pipefail
...
gfortran "${flags[@]}" -c "$script_dir/run_surface_layer_coarse.F90"
gfortran "${flags[@]}" -o run_surface_layer_coarse \
  stub_wrf.o module_sf_mynn.o run_surface_layer_coarse.o
./run_surface_layer_coarse surface-layer-coarse.csv
PYTHONPATH="$repo_root" python3 \
  "$script_dir/validate_coarse_oracle.py" surface-layer-coarse.csv

sha256sum \
  "$source_file" \
  "$script_dir/stub_wrf.F90" \
  "$script_dir/run_surface_layer_coarse.F90" \
  surface-layer-coarse.csv | tee SHA256SUMS_COARSE
