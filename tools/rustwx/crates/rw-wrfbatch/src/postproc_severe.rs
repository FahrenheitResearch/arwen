...
//! - `ehi_0_1km` matches the heavy default: SBCAPE x 0-1 km SRH /
//!   160000 -- emitted under the depth-explicit family name so the
//!   `approx_` suffix names exactly the raw-wrfout diagnostic family
//!   it approximates.
//!
...
pub(crate) fn compute(
    ...
    [
        ...
        ("approx_ehi_0_1km", "dimensionless", ehi),
    ]
    .into_iter()
    .map(|(name, units, values)| SevereField {
        name,
...
mod tests {
    ...
    fn supercell_column_produces_positive_severe_suite() {
        ...
        for slug in ["approx_stp", "approx_scp", "approx_ehi_0_1km"] {
