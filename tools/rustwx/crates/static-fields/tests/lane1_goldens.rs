...
impl Goldens {
    fn load() -> Self {
        ...
        let root = native.as_ref().map(PathBuf::from).unwrap_or_else(||
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("tests").join("goldens").join("lane1"));
        ...
        if native.is_some() {
            assert_eq!(manifest["arithmetic_backend"], "python-numpy",
                       "native authority must be generated by the Python extractor");
            let system = manifest["platform"]["system"].as_str()
                .expect("native authority must identify its platform").to_lowercase();
            assert_eq!(system, std::env::consts::OS,
                       "native authority platform differs from the Rust target");
            assert_eq!(manifest["numpy"], "2.2.6",
                       "lane1 qualifies NumPy 2.2.6; another version needs its own qualification");
            let machine = manifest["platform"]["machine"].as_str()
                .expect("native authority must identify its architecture").to_lowercase();
            ...
            assert_eq!(architecture, std::env::consts::ARCH,
                       "native authority architecture differs from the Rust target");
            assert_eq!(manifest["platform"]["byteorder"], "little",
                       "lane1 binary authorities require little-endian arrays");
    ...
    fn raw(&self, case: &str, key: &str) -> (Vec<usize>, Vec<u8>) {
        ...
        assert!(!self.native_authority || expected_hash.is_some(),
                "native golden payload {case}.{key} must have a Python SHA-256 digest");
        if let Some(expected) = expected_hash {
            use sha2::{Digest, Sha256};
            assert_eq!(format!("{:x}", Sha256::digest(&bytes)), expected,
                       "golden payload {case}.{key} differs from its Python manifest");
        }
        (shape, bytes)
    }
...
#[test]
fn corridor_geometry_cost_equal_python() {
    let g = Goldens::load();
...
#[test]
fn corridor_probes_bit_equal_python() {
    let g = Goldens::load();
    let geometry = CorridorGeometry::derive(2, 1, 3, 18, 15, 45, 39, 51, 44)
        .unwrap();
...
fn npmath_extended_sweep() {
    ...
    let count_mismatches = |observed: &[f32], expected: &[f32], what: &str| {
        ...
        eprintln!("{what}: {bad}/{} bit mismatches", observed.len());
        bad
    };
    ...
    let mut bad = count_mismatches(&sin, &read_f32("sin_out.f32"), "sweep.sin");
    bad += count_mismatches(&cos, &read_f32("cos_out.f32"), "sweep.cos");
    let exp_in = read_f32("exp_in.f32");
    let exp: Vec<f32> = exp_in.iter().map(|&x| np_expf(x)).collect();
    bad += count_mismatches(&exp, &read_f32("exp_out.f32"), "sweep.exp");
    let log_in = read_f32("log_in.f32");
    let log: Vec<f32> = log_in.iter().map(|&x| np_logf(x)).collect();
    bad += count_mismatches(&log, &read_f32("log_out.f32"), "sweep.log");
    assert_eq!(bad, 0, "extended sweep: {bad} bit mismatches");
}
