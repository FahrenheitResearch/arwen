...
fn head_of_clean_checkout() -> Option<String> {
    ...
    //
    // One pathspec, unlike the decoder crates' scripts: nothing under
    // `tools/grib1_bridge` is compiled into this crate, so naming it
    // would make a modified GRIB decoder unstamp a mesh binary it has no
    // part in.
