...
gfortran -c -O2 -ffree-form -ffree-line-length-none \
  ...
  "$script_dir/nucond_production.F90" \
  "$script_dir/radardd02.F90"
gfortran -O2 -o nssl2_nucond_production_oracle \
  module_mp_nssl_2mom.o stub_wrf.o nucond_production.o
gfortran -O2 -o nssl2_radardd02_oracle \
  module_mp_nssl_2mom.o stub_wrf.o radardd02.o

./nssl2_nucond_production_oracle nucond-production.csv | tee oracle.log
./nssl2_radardd02_oracle radardd02.csv | tee -a oracle.log

printf '%s  %s\n' "$actual_source_sha" module_mp_nssl_2mom.F.orig \
  > SOURCE_SHA256
sha256sum nucond-production.csv radardd02.csv | tee ORACLE_SHA256
