program nssl2_graupel_cloud_riming_oracle
  ...
  real :: cloud_diameter, graupel_diameter, graupel_density
  real :: cloud_mass, graupel_mass, before_qv, before_qc, before_nc
  real :: before_qg, before_ng, before_vg, table_temperature, qvs
  ...
  if (len_trim(output_path) == 0) then
     error stop 'usage: nssl2_graupel_cloud_riming_oracle OUTPUT.csv'
  endif
  ...
  write(unit,'(A)') 'case,repetition,dt_s,rho_kg_m3,pressure_pa,exner,temperature_k,cloud_diameter_m,graupel_diameter_m,graupel_density_kg_m3,theta_before_k,qv_before,qc_before,qnc_before_per_kg,qg_before,qng_before_per_kg,qvolg_before_m3_per_kg,theta_after_k,qv_after,qc_after,qnc_after_per_kg,qg_after,qng_after_per_kg,qvolg_after_m3_per_kg,qh_after,qnh_after_per_kg,qvolh_after_m3_per_kg'
  ...
  do k = 1, nz
     do i = 1, nx
        ...
        case (1)
           before_qc = 0.0
        case (2)
           cloud_diameter = 4.0e-6; graupel_diameter = 300.0e-6
           graupel_density = 200.0
        case (3)
           cloud_diameter = 8.0e-6; graupel_diameter = 300.0e-6
           graupel_density = 300.0
        case (4)
           cloud_diameter = 15.0e-6; graupel_diameter = 500.0e-6
           graupel_density = 500.0
        case (5)
           cloud_diameter = 30.0e-6; graupel_diameter = 1.0e-3
           graupel_density = 800.0
        case (6)
           cloud_diameter = 50.0e-6; graupel_diameter = 2.0e-3
           graupel_density = 170.0
        case (7)
           cloud_diameter = 60.0e-6; graupel_diameter = 3.0e-3
           graupel_density = 900.0
        case (8)
           cloud_diameter = 70.0e-6; graupel_diameter = 8.0e-3
           graupel_density = 500.0
        case (9)
           cloud_diameter = 25.0e-6; graupel_diameter = 700.0e-6
           graupel_density = 300.0; before_qc = 1.0e-6; before_qg = 2.0e-3
        case (10)
           cloud_diameter = 50.0e-6; graupel_diameter = 2.0e-3
           graupel_density = 600.0; before_qc = 1.0e-3; before_qg = 1.0e-6
        case default
           cloud_diameter = 80.0e-6; graupel_diameter = 5.0e-3
           graupel_density = 700.0; before_qc = 2.0e-3; before_qg = 5.0e-3
        end select
  ...
  enddo
  close(unit)
  print '(A,1X,A)', 'NSSL2_GRAUPEL_CLOUD_RIMING_ORACLE_COMPLETE', &
       trim(output_path)
...
contains
  ...
  subroutine run_cell(output_unit)
    ...
    write(output_unit,'(2(I0,","),25(ES24.16E3,","),ES24.16E3)') &
         case_id, repetition, dt, rho, pressure, exner, temperature, &
         cloud_diameter, graupel_diameter, graupel_density, &
         theta_before, before_qv, before_qc, before_nc, before_qg, &
         before_ng, before_vg, a1(1,1,2,lt), a1(1,1,2,lv), &
         a1(1,1,2,lc), a1(1,1,2,lnc)/rho, a1(1,1,2,lh), &
         a1(1,1,2,lnh)/rho, a1(1,1,2,lvh)/rho, a1(1,1,2,lhl), &
         a1(1,1,2,lnhl)/rho, a1(1,1,2,lvhl)/rho
  end subroutine run_cell
end program nssl2_graupel_cloud_riming_oracle
