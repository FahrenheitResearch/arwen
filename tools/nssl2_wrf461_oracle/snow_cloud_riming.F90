program nssl2_snow_cloud_riming_oracle
  ...
  integer, parameter :: lnc = 10, lns = 13, lng = 14
  integer :: i, k, unit, nml_unit, case_id, repetition, table_index
  real :: nssl_params(20), dt, temperature, pressure, exner, rho
  real :: cloud_diameter, snow_diameter, cloud_mass, snow_mass
  real :: before_qv, before_qc, before_nc, before_qs, before_ns
  ...
  if (len_trim(output_path) == 0) then
     error stop 'usage: nssl2_snow_cloud_riming_oracle OUTPUT.csv'
  endif
  ...
  write(unit,'(A)') 'case,repetition,dt_s,rho_kg_m3,pressure_pa,exner,temperature_k,cloud_diameter_m,snow_diameter_m,theta_before_k,qv_before,qc_before,qnc_before_per_kg,qs_before,qns_before_per_kg,theta_after_k,qv_after,qc_after,qnc_after_per_kg,qs_after,qns_after_per_kg,qg_after,qng_after_per_kg'
  ...
  do k = 1, nz
     do i = 1, nx
        ...
        temperature = 250.0 + 0.5*real(mod(case_id, 8))
        ...
        case (1)
           before_qc = 0.0
        case (2)
           cloud_diameter = 8.0e-6; snow_diameter = 20.0e-6
        case (3)
           cloud_diameter = 15.0e-6; snow_diameter = 80.0e-6
        case (4)
           cloud_diameter = 30.0e-6; snow_diameter = 200.0e-6
        case (5)
           cloud_diameter = 40.0e-6; snow_diameter = 500.0e-6
        case (6)
           cloud_diameter = 50.0e-6; snow_diameter = 1.0e-3
        case (7)
           cloud_diameter = 60.0e-6; snow_diameter = 3.0e-3
        case (8)
           cloud_diameter = 70.0e-6; snow_diameter = 8.0e-3
        case (9)
           cloud_diameter = 25.0e-6; snow_diameter = 700.0e-6
           before_qc = 1.0e-6; before_qs = 2.0e-3
        case (10)
           cloud_diameter = 50.0e-6; snow_diameter = 2.0e-3
           before_qc = 1.0e-3; before_qs = 1.0e-6
        case default
           cloud_diameter = 80.0e-6; snow_diameter = 5.0e-3
           before_qc = 2.0e-3; before_qs = 5.0e-3
        end select
  ...
  enddo
  close(unit)
  print '(A,1X,A)', 'NSSL2_SNOW_CLOUD_RIMING_ORACLE_COMPLETE', &
       trim(output_path)
...
contains
  ...
  subroutine run_cell(output_unit)
    ...
    write(output_unit,'(2(I0,","),20(ES24.16E3,","),ES24.16E3)') &
         ...
         cloud_diameter, snow_diameter, theta_before, before_qv, &
         before_qc, before_nc, before_qs, before_ns, a1(1,1,2,lt), &
         a1(1,1,2,lv), a1(1,1,2,lc), a1(1,1,2,lnc)/rho, &
         a1(1,1,2,ls), a1(1,1,2,lns)/rho, a1(1,1,2,lh), &
         a1(1,1,2,lng)/rho
  end subroutine run_cell
end program nssl2_snow_cloud_riming_oracle
