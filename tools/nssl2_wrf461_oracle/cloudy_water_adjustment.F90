program nssl2_cloudy_water_adjustment_oracle
  ...
  if (len_trim(output_path) == 0) then
     error stop 'usage: nssl2_cloudy_water_adjustment_oracle OUTPUT.csv'
  endif
  ...
  write(unit,'(A)') 'case,i,k,dt_s,pressure_pa,exner,rho_kg_m3,temperature_k,supersaturation_percent,qv_before,theta_before_k,qc_before,nc_before_per_kg,qnn_before_per_kg,qv_after,theta_after_k,qc_after,nc_after_per_kg,qnn_after_per_kg'
  ...
  do k = 1, nz
     do i = 1, nx
        ...
        case (0)
           ...
           qc_before = 1.0e-7; nc_before = 2.0e8
           qnn_before = 0.20*(0.5e9/1.225); dt = 1.0
        case (1)
           supersaturation_percent = -5.0
           qc_before = 5.0e-5; nc_before = 1.5e8
           qnn_before = 0.50*(0.5e9/1.225); dt = 5.0
        case (2)
           supersaturation_percent = -1.0
           qc_before = 5.0e-4; nc_before = 2.5e8
           qnn_before = 0.75*(0.5e9/1.225); dt = 10.0
        case (3)
           supersaturation_percent = -0.1
           qc_before = 1.0e-4; nc_before = 3.0e8
           qnn_before = 0.90*(0.5e9/1.225); dt = 30.0
        case (4)
           supersaturation_percent = 0.0
           qc_before = 1.0e-4; nc_before = 2.0e8
           qnn_before = 1.00*(0.5e9/1.225); dt = 60.0
        case (5)
           supersaturation_percent = 0.01
           qc_before = 1.0e-4; nc_before = 1.0e8
           qnn_before = 0.80*(0.5e9/1.225); dt = 1.0
        case (6)
           supersaturation_percent = 0.10
           qc_before = 2.0e-4; nc_before = 2.0e8
           qnn_before = 0.60*(0.5e9/1.225); dt = 5.0
        case (7)
           supersaturation_percent = 0.49
           qc_before = 3.0e-4; nc_before = 3.5e8
           qnn_before = 0.40*(0.5e9/1.225); dt = 10.0
        case (8)
           supersaturation_percent = 0.50
           qc_before = 5.0e-5; nc_before = 5.0e7
           qnn_before = 0.25*(0.5e9/1.225); dt = 30.0
        case (9)
           supersaturation_percent = -0.01
           qc_before = 1.0e-12; nc_before = 1.0e8
           qnn_before = 0.10*(0.5e9/1.225); dt = 60.0
        case (10)
           supersaturation_percent = -0.50
           qc_before = 1.0e-3; nc_before = 1.0e12
           qnn_before = 0.90*(0.5e9/1.225); dt = 1.0
        case default
           supersaturation_percent = 0.20
           qc_before = 1.0e-5; nc_before = 1.0e3
           qnn_before = 0.70*(0.5e9/1.225); dt = 5.0
        end select

        temperature = 268.0 + 6.25*real(mod(k-1,4)) + 0.17*real(i-1)
        ...
        write(unit,'(3(I0,","),15(ES24.16E3,","),ES24.16E3)') &
             case_id, i, k, dt, pressure, exner, rho, temperature, &
             supersaturation_percent, qv_before, theta_before, qc_before, &
             nc_before, qnn_before, qv_after, theta_after, qc_after, &
             nc_after, qnn_after
     enddo
  enddo
  close(unit)

  print '(A,1X,A)', 'NSSL2_CLOUDY_WATER_ADJUSTMENT_ORACLE_COMPLETE', &
       trim(output_path)
...
end program nssl2_cloudy_water_adjustment_oracle
