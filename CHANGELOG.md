...
Fixed:
- A nested forecast under `[tiles]` set to `auto`, the cyclone quick-start included, runs resident when the whole configured tree fits the card, instead of being refused by the tile planner's shared process and radiation floor charged before any domain is priced (the reported case: a 12/3 km moving-nest cyclone refused while the same tree needed 3,950,025,504 bytes resident against a 6,855,065,600 byte admission budget, and ran). The tree road now asks the resident question first, as the single-domain road already did, and consults the planner only where that answer is no. The refusal that remains names the resident envelope, the tile floor and the budget in one sentence and the way out in the next, and no longer tells the user to re-run with `--tiles off`; where a domain's own `mode = "on"` is what put a fitting tree on the tiled road, the refusal names that table and says to set it to `auto`, and a refusal raised on a tiling rather than on memory keeps the planner's own sentence and points at the geometry instead of at the card. The plan review and the run door now price the admission from one shared estimate, so a tree the review admits is not refused at run start after the download. An automatic tree that moves a nest withholds that nest's rebuild from the admission budget and records the pinned host snapshot the move stages through. The cyclone document carries a `streaming` block saying which road each domain takes.
