...
| `--cycle YYYY-MM-DDTHH|latest` | model cycle (UTC); 'latest' resolves the newest complete cycle from the AWS Open Data listing (gfs/hrrr only -- the table routes read a packaged cycle grammar rather than a live listing, and publication lag differs by hours between those producers, so they want an explicit cycle) |
