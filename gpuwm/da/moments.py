...
def _repair_authority(mp_physics: int, token) -> str | None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"mp_physics={mp_physics} and gpuwm.da.moments knows no such "
...
def _static_schemes_from_the_registry() -> dict[int, SchemeMoments]:
    ...
    for spelled in (_KESSLER, _WSM6, _THOMPSON, _MORRISON):
        ...
        if derived != spelled:
            raise RuntimeError(
                f"gpuwm.da.moments spells mp_physics={spelled.mp_physics} as "
...
def scheme_moments(mp_physics: int, **selectors) -> SchemeMoments:
    ...
    if key not in _STATIC_SCHEMES:
        raise MomentPolicyError(
            ...
            "is derived from gpuwm/physics_registry_v2.json "
