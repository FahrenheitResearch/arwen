...
def build_static(
    ...
    below, which is silent unless ``GPUWM_PERF_TIMING`` is set -- and it
    ...
    (proven byte-identical to the numpy reference against goldens
    generated by running that reference).  ``GPUWM_STATIC_PYTHON=1``
...
def _build_static_routed(
    ...
    The fallback (explicit ``GPUWM_STATIC_PYTHON=1``, an unloadable
    library, or a grid object that is not a ProjectedGrid and so cannot
    cross the seam) is reported once per operation by
    :func:`gpuwm.static.rust_bridge.route`.  With the bridge active, a
    ...
    if selection.root != root:
        raise ValueError(
            ...
            f"geog_root {root}")
    handle = grid._rust_handle(bridge)
