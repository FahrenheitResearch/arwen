...
| `gpuwm enprod` (ensemble products) | **Experimental** | Green over an ensemble. `--make-fixture` writes a synthetic one, but **drawing it needs the render extra** -- `pip install 'gpuwm[render]'` -- so on a base install the fixture builds and then `gpuwm enprod ENS_ROOT` refuses with zero PNGs. Unlike `gpuwm render` there is no matplotlib fallback. Member generation is undocumented: no public page prints a runnable command that produces members. |
